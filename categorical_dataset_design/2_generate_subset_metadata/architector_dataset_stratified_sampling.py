...
def _sample_strata_worker(args):
    ...
    if start_byte > 0:
        with open(jsonl_file_path, 'r') as f:
            f.seek(start_byte)
            # Read and discard the first line to align with the start_byte
            f.readline()
            global_idx_start = sum(1 for _ in f)
...
if __name__ == "__main__":
    ...
    jsonl_file_path = input_data["jsonl_file_path"]
    n_points = int(input_data["n_points"])
    model_terms = input_data.get("model_terms", None)
    ligand_types = input_data.get("ligand_types", None)
    ligand_subtypes = input_data.get("ligand_subtypes", ['W', 'N', 'D'])
    include_variant_counts = input_data.get("include_variant_counts", False)
    seed = int(input_data.get("seed", 123))
    previous_design_path = input_data.get("previous_design", None)
    output_file = input_data["output_file"]
    weighting = input_data.get("weighting", "sqrt")
    min_per_stratum = int(input_data.get("min_per_stratum", 1))
    chunk_size = int(input_data.get("chunk_size", 100000))
    max_samples_per_stratum = int(input_data.get("max_samples_per_stratum", 10000))
    ...
    previous_design = None
    if previous_design_path:
        previous_design = pd.read_csv(previous_design_path)
    ...
    print(f"\nSelected design saved to {output_file}")
